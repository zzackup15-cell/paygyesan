...
| `/busuip` | 부수입 세금 계산기 (종합소득세·5월 정산) |
| `/boheom` | 4대보험 계산기 (근로자·사업주 부담) |
| `/yeongeum` | 국민연금 예상 수령액 계산기 (노령연금) |
| `/juhyu` | 주휴수당 계산기 |
...
│     ├─ busuip.js   부수입 세금 계산기 UI
│     ├─ boheom.js   4대보험 계산기 UI
│     ├─ yeongeum.js 국민연금 예상 수령액 계산기 UI
...
│  ├─ test/          검증 스위트 (node tools/test/run.js)
...
### 국민연금 노령연금 (국민연금법 제51조)

```
기본연금액(연액) = Σ[ 상수ᵢ × (A + wᵢ·B) × Pᵢ/P ] × (1 + 0.05 × n/12)
월 연금액 = 기본연금액 ÷ 12 × 가입기간 지급률 × 수령시기 지급률
```

A는 전체 가입자 평균소득월액의 3년간 평균(2026년 **3,193,511원**), B는 본인의 가입기간 중 기준소득월액 평균, Pᵢ는 해당 연도 가입월수, n은 20년(240개월) 초과 가입월수입니다.

**비례상수는 가입한 연도마다 다릅니다.** 각 연도 가입월수의 비중으로 가중합산하므로, 1990년대 가입기간이 있으면 그 기간에는 상수 2.4가 그대로 적용됩니다.
...
| 1999~2007년 | 1.8 | 60% |
| 2008년 | 1.5 | 50% |
| 2009~2025년 | 매년 0.015 감소 (2025년 1.245) | 매년 0.5%p 감소 |
| 2026년 이후 | 1.29 | 43% (2025년 개혁으로 고정) |

Σ의 가중치 Pᵢ/P는 합이 1이라 **총 가입기간이 기본연금액에 반영되지 않습니다.** 상수 자체가 20년 가입을 전제로 만들어졌기 때문입니다. 20년을 넘는 부분은 `(1 + 0.05n/12)`가 받고, 못 미치는 부분은 가입기간 지급률(10년 50%, 1년마다 5%p, 20년 100%)이 받습니다. 이 지급률을 빠뜨리면 10년 가입자와 20년 가입자의 연금액이 같아집니다.

수령시기 지급률은 조기수급 1년당 6% 감액(최대 5년 70%), 연기연금 1년당 7.2% 가산(최대 5년 136%)입니다. 수급개시연령은 출생연도별 60~65세이며 최소 가입기간은 120개월입니다.

B값은 가입 이력이 필요한 유일한 값이지만, 공단이 과거 소득을 재평가율로 현재가치 환산하므로 **현재 소득으로 대체**합니다. 결과는 모두 현재가치 기준입니다.

### 부수입 세금 (종합소득세)

```
추가 세금 = 종합소득세(기존 과세표준 + 부수입 소득금액) − 종합소득세(기존 과세표준)
           + 지방소득세(추가 소득세의 10%)
```

**총액이 아니라 차이를 계산합니다.** 정확한 종합소득세는 개인별 공제에 좌우되지만, 두 과세표준의 세액 차이로 구하면 양쪽에 똑같이 걸리는 공제가 상쇄됩니다. 기존 과세표준은 총급여에서 근로소득공제(소득세법 제47조)·인적공제·4대보험료공제만 빼서 추정합니다.
...
소득 유형에 따라 갈립니다. **사업소득**은 3.3% 원천징수에 실제 경비만 인정되고 금액과 무관하게 종합과세됩니다. **기타소득**은 8.8% 원천징수지만 필요경비 60%를 의제로 인정하고, 기타소득금액 300만 원(지급액 750만 원) 이하면 분리과세를 선택할 수 있습니다.

원천징수는 정산이 아니라 예납이므로 `추가 세금 − 원천징수액`이 5월에 정산할 금액입니다. 한계세율이 15% 이상이면 3.3%만 뗀 사업소득은 반드시 추가 납부가 생깁니다.

근로 외 소득이 연 2,000만 원을 넘으면 건강보험 소득월액보험료가 붙습니다.

```
소득월액보험료 = (연간 보수외소득 − 2,000만 원) ÷ 12 × 소득평가율 × 건강보험료율
```

사업·기타소득의 소득평가율은 100%입니다. 보수월액보험료와 달리 **사업주 부담이 없어 요율 전체를 본인이 냅니다**(국민건강보험법 제69조·제76조). 장기요양보험료도 함께 붙습니다.
...
| `pension.js` | 국민연금 상수·지급률·조기연기·수급개시연령 | 133 |
| `sideincome.js` | 종합소득세율·근로소득공제·기타소득·소득월액보험료 | 123 |
...
`pension.js` 와 `sideincome.js` 에는 각각 중심 검증이 하나씩 있습니다.
국민연금은 **평균소득자가 40년 가입하면 소득대체율이 정확히 43%** 여야 합니다.
'소득대체율 43%' 라는 말의 정의가 그것이기 때문입니다. 나아가 대체율은
가입연수에 정비례해야 하며(43% × 연수/40), 10년부터 40년까지 전 구간을 그
이론값과 대조합니다. 이 검증이 20년 미만 지급률 누락을 잡았습니다.
종합소득세는 **8개 구간의 경계값이 국세청 고시표와 한 자리도 달라선 안 되고**,
누진공제가 맞으면 구간 경계에서 세액이 튀지 않아야 합니다.
