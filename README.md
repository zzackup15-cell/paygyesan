...
한국 근로자를 위한 통상임금·통상시급·연봉 환산 계산기입니다. 빌드 도구·서버·DB 없이 순수 HTML/CSS/JS로 동작하며, Cloudflare Pages 정적 호스팅을 전제로 합니다.
...
Node에서 `Calc`를 직접 불러 35개 케이스를 검증했고 전부 통과했습니다. 같은 값이 실제 브라우저 DOM에서도 나오는 것까지 확인했습니다.
