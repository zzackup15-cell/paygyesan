...
Node에서 `Calc`를 직접 불러 179개 케이스(통상임금 35 + 세금 32 + 퇴직금 34 + 연차 35 + 실업급여 43)를 검증했고 전부 통과했습니다. 주휴수당 계산기도 같은 `Calc` 를 쓰므로 이 검증을 공유합니다. 같은 값이 실제 브라우저 DOM에서도 나오는 것까지 확인했습니다.
